@testset "Open BC" begin
    ...
    l = @inferred accumulate_left(f, Open)
    r = @inferred accumulate_right(f, Open)
    m = @inferred accumulate_middle(f, Open)
    ...
    @testset "Accumulate left" begin
        l_exhaust = OffsetArray([zeros(1, q) for q in nstates(f, Open)], -1)
    ...
    @testset "Accumulate right" begin
        r_exhaust = OffsetArray([zeros(q, 1) for q in nstates(f, Open)], +1)
        for i in Iterators.take(eachindex(r_exhaust), length(chain)-1)
            X = Iterators.product((1:q for q in Iterators.drop(nstates(chain), i-2))...)
            for x in X
                r_exhaust[i][first(x)] += evaluate_partial(chain, x, i-1, length(chain)-1)
            end
    ...
    @testset "Accumulate middle" begin
        m_exhaust = OffsetArray([zeros(q1, q2) for q1 in collect(nstates(f, Open))[1:end-1], q2 in collect(nstates(f, Open))[2:end]], 0, +1) 
